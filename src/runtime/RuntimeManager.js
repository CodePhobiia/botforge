...
class RuntimeManager {
    constructor({ legacyManager, getLatestBotStatusEvent, listBotsByUser, listAllBots, logger } = {}) {
    ...
    async startBot(botId, { config = null } = {}) {
        ...
        if (resolved && this.isOpenClaw(resolved)) {
            const event = {
                ...
                message: buildLifecycleMessage('start')
            };
            return {
                executed: false,
                eventType: event.eventType,
                message: event.message,
                status: this._buildOpenClawStatus(resolved, { event })
            };
        }
    ...
    async stopBot(botId, { config = null } = {}) {
        ...
        if (resolved && this.isOpenClaw(resolved)) {
            const event = {
                ...
                createdAt: new Date().toISOString(),
                message: buildLifecycleMessage('stop')
            };
            return {
                executed: false,
                eventType: event.eventType,
    ...
    async removeBot(botId, { config = null } = {}) {
        const resolved = this.getConfig(botId, config);
        if (resolved && this.isLegacy(resolved) && this.legacyManager?.bots?.has(botId)) {
    ...
    async updateBot(botId, nextConfig, { previousConfig = null } = {}) {
        ...
        if (previousRuntime === RUNTIMES.LEGACY && nextRuntime === RUNTIMES.OPENCLAW) {
            ...
            return {
                runtimeChanged: true,
                status: this._buildOpenClawStatus(next)
            };
        }
    ...
    updateBotConfig(botId, updates, { config = null } = {}) {
        const resolved = this.getConfig(botId, config);
        if (resolved && this.isOpenClaw(resolved)) {
